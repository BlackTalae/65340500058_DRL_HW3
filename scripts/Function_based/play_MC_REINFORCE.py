...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    episode = 3000
    w_file = f"{Algorithm_name}_{episode}_{agent.num_of_action}_{agent.action_range[1]}_{agent.discount_factor}_{agent.lr}_{agent.hidden}.pth"
        ...
        with torch.inference_mode():
            ...
            for episode in range(n_episodes):
                ...
                while not done:
                    ...
                    # agent stepping
                    action_probs = agent.policy_net(obs)
    ...
    obs_file_path = os.path.join("saved_obs", f"{Algorithm_name}_{agent.num_of_action}_{agent.action_range[1]}_{agent.discount_factor}_{agent.lr}_{agent.hidden}.json")
    ...
    plt.title(f"{Algorithm_name}_{agent.num_of_action}_{agent.action_range[1]}_{agent.discount_factor}_{agent.lr}_{agent.hidden}")
    plt.ylim(-0.4, 0.4)
    plt.legend()
    plt.grid(True)
    ...
    save_path = os.path.join(save_dir, f"3_{Algorithm_name}_{agent.num_of_action}_{agent.action_range[1]}_{agent.discount_factor}_{agent.lr}_{agent.hidden}.png")
